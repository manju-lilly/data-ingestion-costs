...
response = STS_CLIENT.assume_role(RoleArn=edbroleArn, RoleSessionName = "test_edb")
...
def handler(event, context):
    ...
    delta_files = ['mdit/fda/delta/2020/10/29/part-00000-be415fdb-ee07-4cb7-95ec-e67e53ec2e8d-c000.csv']
    data = []
...
def read_delta_file(delta_file_path):
    ...
    try:
        ...
        return 
        csv_reader = csv.DictReader((line for line in lines if not line.isspace() and not line.replace(",", "").isspace()),delimiter=',')
        ...
        for row in list(csv_reader):
            ...
            if row['path'].endswith("cfm"):
                ...
                inner_pdf_paths = get_s3_objects(bucket_name, prefix=key, suffix=".pdf")
        ...
        print(e)
        logger.exception(f"failed to read contents of the file: {delta_file_path}")
...
def write_to_s3(all_records,filename):
    ...
    with open(filename, mode='w',newline="") as csv_file:
        ...
        for record in all_records:
            row = [record['appplication_docs_type_id'], record['application_no'],record['submission_type'],record['submission_no'],record['application_docs_url'],record['drug_name'],record['s3_path'],record['url'],record['filename'], record['num_pages']]
            print(row)
